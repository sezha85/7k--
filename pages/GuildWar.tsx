...
const SAMPLE_ENEMY_DECKS: EnemyDeck[] = [
  ...
  },
  {
    ...
    front: ['오목', '프레나'],
    back: ['실베스타'],
    memo: '오목1 → 실베2 → 실베1 구도 주의. 펫 연지 가능성 체크.',
  },
];
...
const GuildWar: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const loadedEnemies = enemySnapshot.docs.map(item => ({ id: item.id, ...item.data() })) as EnemyDeck[];
      const loadedCounters = counterSnapshot.docs.map(item => ({ id: item.id, ...item.data() })) as CounterDeck[];
      ...
      if (localEnemies) setEnemyDecks(JSON.parse(localEnemies));
      if (localCounters) setCounterDecks(JSON.parse(localCounters));
  ...
  const deleteEnemyDeck = async (id: string) => {
    ...
    try {
      ...
      if (dbMode === 'firebase') {
        await Promise.all(linkedCounters.map(item => deleteDoc(doc(db, 'guild_counter_decks', item.id))));
      }
  ...
  return (
    <div className="min-h-screen bg-dark text-slate-100">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xl sticky top-16 z-30">
          <div className="relative">
            ...
            <input
              ...
              placeholder="상대 영웅 입력 예: 린 오"
              className="w-full bg-slate-950 border border-slate-700 rounded-2xl py-4 pl-12 pr-4 text-lg text-white placeholder:text-slate-600 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
            />
          </div>
        ...
        {message && (
          <div className="flex items-start gap-2 bg-amber-950/30 border border-amber-900/50 text-amber-200 rounded-xl p-3 text-sm">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex flex-wrap gap-2 mb-3">
                          {enemy.heroes.map(hero => (
                            <span key={hero} className="bg-red-950/50 border border-red-900/60 text-red-100 rounded-xl px-3 py-1.5 font-bold text-sm">
                              {hero}
                            </span>
                          ))}
                        </div>
                      ...
                      <button onClick={() => copyDeckText(enemy, counters)} className="shrink-0 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl px-3 py-2 text-xs font-bold border border-slate-700">
                        복사
                      </button>
                    </div>
                    {(enemy.front.length > 0 || enemy.back.length > 0 || enemy.memo) && (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {isAdmin && (
                        <div className="flex gap-2">
                          <button onClick={() => editEnemy(enemy)} className="text-xs bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg px-2 py-1 text-slate-300 flex items-center gap-1"><Edit3 size={12} />상대 수정</button>
                          <button onClick={() => deleteEnemyDeck(enemy.id)} className="text-xs bg-red-950/40 hover:bg-red-900/50 border border-red-900 rounded-lg px-2 py-1 text-red-200 flex items-center gap-1"><Trash2 size={12} />삭제</button>
                        </div>
                      )}
                    </div>

                    {counters.length === 0 ? (
                      <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 text-sm text-slate-500">등록된 카운터덱이 없습니다.</div>
                    ) : (
                      counters.map(counter => (
                        <div key={counter.id} className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 space-y-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {counter.heroes.map(hero => <span key={hero} className="bg-primary/10 border border-primary/30 text-amber-100 rounded-xl px-3 py-1 text-sm font-bold">{hero}</span>)}
                              </div>
                            </div>
                            {isAdmin && (
                              <div className="flex gap-2 shrink-0">
                                <button onClick={() => editCounter(counter)} className="text-xs bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg px-2 py-1 text-slate-300"><Edit3 size={12} /></button>
                                <button onClick={() => deleteCounterDeck(counter.id)} className="text-xs bg-red-950/40 hover:bg-red-900/50 border border-red-900 rounded-lg px-2 py-1 text-red-200"><Trash2 size={12} /></button>
                              </div>
                            )}
                          </div>
                          ...
                          {counter.skillOrder && (
                            <div>
                              <div className="text-xs text-blue-400 font-bold mb-2">스킬 순서</div>
                              <div className="flex flex-wrap gap-y-2 items-center text-sm">
                                {skillParts(counter.skillOrder).map((skill, index, arr) => (
                                  <React.Fragment key={`${skill}-${index}`}>
                                    <span className="bg-slate-900 border border-slate-700 rounded-lg px-2 py-1 text-slate-100 font-bold">{skill}</span>
                                    {index < arr.length - 1 && <ChevronRight size={14} className="text-slate-600 mx-1" />}
                                  </React.Fragment>
                                ))}
                              </div>
                            </div>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
          {!isAdmin ? (
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
              <div>
                <h2 className="font-black text-white">관리자 모드</h2>
                <p className="text-sm text-slate-500">상대덱과 카운터덱을 입력, 수정, 삭제합니다.</p>
              </div>
              <div className="flex gap-2">
                <input
                  value={adminPassword}
                  ...
                  className="bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-primary"
                />
                <button onClick={handleAdminLogin} className="bg-primary hover:bg-amber-600 text-white font-bold rounded-xl px-4 py-2 text-sm">열기</button>
              </div>
            </div>
          ) : (
            <div className="space-y-8">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h2 className="font-black text-white text-xl">관리자 입력</h2>
                  <p className="text-sm text-slate-500">영웅명은 띄어쓰기, /, 쉼표로 구분 가능합니다.</p>
                </div>
                <button onClick={() => setIsAdmin(false)} className="bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-300 flex items-center gap-1"><X size={14} />닫기</button>
              </div>

              <FormPanel title="상대덱 등록 / 수정" onSave={saveEnemyDeck} onReset={() => setEnemyForm(emptyEnemyForm)} isEditing={!!enemyForm.id}>
                <DeckForm form={enemyForm} setForm={setEnemyForm} mode="enemy" enemyDecks={enemyDecks} />
              </FormPanel>

              <FormPanel title="카운터덱 등록 / 수정" onSave={saveCounterDeck} onReset={() => setCounterForm(emptyCounterForm)} isEditing={!!counterForm.id}>
                <DeckForm form={counterForm} setForm={setCounterForm} mode="counter" enemyDecks={enemyDecks} />
              </FormPanel>
            </div>
          )}
        </section>
...
const InfoBox: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-3">
    ...
    <div className="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed">{value}</div>
  </div>
);
...
const FormPanel: React.FC<{ title: string; children: React.ReactNode; onSave: () => void; onReset: () => void; isEditing: boolean }> = ({ title, children, onSave, onReset, isEditing }) => (
  <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-4 space-y-4">
    <div className="flex items-center justify-between">
      ...
      {isEditing && <span className="text-xs bg-blue-950/50 border border-blue-900 text-blue-200 rounded-full px-3 py-1">수정 중</span>}
    </div>
    {children}
    <div className="flex gap-2 justify-end">
      <button onClick={onReset} className="bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl px-4 py-2 text-sm text-slate-300 flex items-center gap-1"><RotateCcw size={14} />초기화</button>
      <button onClick={onSave} className="bg-primary hover:bg-amber-600 rounded-xl px-4 py-2 text-sm text-white font-bold flex items-center gap-1"><Save size={14} />저장</button>
    </div>
  </div>
);
    ...
    <div className="grid sm:grid-cols-2 gap-3">
      {mode === 'counter' && (
        <label className="space-y-1 sm:col-span-2">
          <span className="text-xs text-slate-500 font-bold">연결할 상대덱</span>
          <select value={form.enemyDeckId || ''} onChange={event => setValue('enemyDeckId', event.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-3 text-white focus:outline-none focus:border-primary">
            <option value="">상대덱 선택</option>
            {enemyDecks.map(deck => <option key={deck.id} value={deck.id}>{deck.name} ({joinList(deck.heroes)})</option>)}
          </select>
        </label>
      )}
      ...
      <label className="space-y-1">
        <span className="text-xs text-slate-500 font-bold">진형</span>
        <select value={form.formation} onChange={event => setForm(prev => ({ ...prev, formation: event.target.value as FormationName }))} className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-3 text-white focus:outline-none focus:border-primary">
          {Object.entries(FORMATIONS).map(([name, info]) => <option key={name} value={name}>{name} ({info.description})</option>)}
        </select>
      </label>
      ...
      <label className="space-y-1 sm:col-span-2">
        <span className="text-xs text-slate-500 font-bold">참고사항 / 메모</span>
        <textarea value={form.memo} onChange={event => setValue('memo', event.target.value)} placeholder="장비, 속공순서, 펫, 주의 변수 등을 입력" className="w-full min-h-28 bg-slate-900 border border-slate-700 rounded-xl px-3 py-3 text-white focus:outline-none focus:border-primary resize-y" />
      </label>
    </div>
  );
};
...
const TextField: React.FC<{ label: string; value: string; onChange: (value: string) => void; placeholder?: string }> = ({ label, value, onChange, placeholder }) => (
  <label className="space-y-1">
    ...
    <input value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-3 text-white placeholder:text-slate-600 focus:outline-none focus:border-primary" />
  </label>
);
